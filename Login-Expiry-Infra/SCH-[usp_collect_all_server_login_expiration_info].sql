...
/*
	Purpose:		Gather login metrics like last_password_set_date, expiry_date etc from each SQLServer using Linked Server object.
					[DBA_Admin].[dbo].[all_server_login_expiry] is target table.
	...
	Modifications:	2024-04-01 - Sushant - Initial Draft
					2024-07-04 - Ajay - Cleanup & Meta Data Addition

	Examples:	
		exec usp_collect_all_server_login_expiration_info @verbose = 2, @execute = 0, @test_server = '10.253.33.32'				
		exec usp_collect_all_server_login_expiration_info @verbose = 2, @execute = 1, @test_server = '10.253.33.32'
		exec usp_collect_all_server_login_expiration_info @verbose = 2, @execute = 1, @test_server = '10.253.33.32', @generate_error_scenario = 1
*/
	SET NOCOUNT ON;
...
/*
	...
	CREATE TABLE login_email_mapping
	(
		sql_instance_ip		varchar(20),		-- should match with link server name
		sql_instance_name	varchar(125),	-- serverproprty(servername)
		server_alias_name	varchar(125),
		at_server_name		varchar(125),
		host_name			varchar(125),
		login_name			varchar(125),
		owner_group_email	varchar(4000),
	...
	CREATE UNIQUE CLUSTERED INDEX uci_ip_login on login_email_mapping (sql_instance_ip, login_name)
*/
END
GO
