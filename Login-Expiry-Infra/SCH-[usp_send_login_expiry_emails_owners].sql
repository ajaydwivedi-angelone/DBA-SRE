...
CREATE OR ALTER PROCEDURE dbo.usp_send_login_expiry_emails
	...
	@dba_team_email_id varchar(125) = 'bi.dba@angelbroking.com',
	@dba_manager_threshold_days int = 7,
	@dba_manager_email_id varchar(125) = 'Yashwanth.1Kotari@angelbroking.com', /* Email for DBA Manager */
	@copy_dba_team_for_all_mails bit = 0, /* 1 = Copy dba team for all email notifications */
	@sre_vp_threshold_days int = 7,
	@sre_vp_email_id varchar(125) = 'hemanthkumar.m@angelbroking.com', /* Email for SRE Senior VP */
	@cto_threshold_days int = 3,
	@cto_email_id varchar(125) = 'jyotiswarup.raiturkar@angelbroking.com', /* Email for CTO */
	@url_login_expiry_dashboard_panel varchar(1000) = 'https://internalstatus.angelbroking.com/grafana-prod/d/distributed_live_dashboard_all_servers/monitoring-live-all-servers?orgId=1&refresh=1m&viewPanel=885',
...
/*
	Purpose:		Send mail notification to login owners
					...
					https://angelbrokingpl.atlassian.net/browse/SRE-24330
					https://angelbrokingpl.atlassian.net/browse/DB-1231

	Modifications:	2024-04-01 - Sushant - Initial Draft
					2024-07-18 - Ajay - Update as per suggestions of management

	Examples:	
		exec dbo.usp_send_login_expiry_emails @verbose = 2, @execute = 0, @test_server = '10.253.33.32'				
...
*/
	...
	declare @_mail_subject varchar(500)
	declare @_mail_html nvarchar(max)
	declare @_mail_html_title nvarchar(2000);
	...
	select	lei.collection_time, lei.sql_instance, lei.login_name, lei.is_sysadmin, lei.password_last_set_time,
			...
			[login_owner_group_email] = lem.owner_group_email, ct.server_owner_email,
	...
	begin
		...
		begin
			...
			if	(	left(lower(@c_login_name),6) = 'bidba.'
			...
			begin
				...
				SELECT @c_sql_instance, @c_login_name, @dba_team_email_id
			end
			else -- Non-DBA login
			begin
	...
	begin
		...
		begin
			...
			begin try
				...
				set @_table_headline = N'<h3><a href="'+@url_login_expiry_dashboard_panel+'" target="_blank">ATTENTION - Following Login Password Are Expiring</a></h3>'+@_crlf;
	...
	begin
		...
		begin try
			...
			;with t_login_info as (
				...
				inner join dbo.all_server_login_expiry_info_dashboard lei
					on lei.sql_instance = lex.sql_instance and lei.login_name = lex.login_name
				where lei.days_until_expiration <= @sre_vp_threshold_days
				and left(lower(lex.login_name),6) <> 'bidba.'
			)
			,t_table_rows as (
				select	'<tr>'
						+'<td class="bg_key">'+sql_instance+'</td>'
						+'<td class="bg_key">'+login_name+'</td>'
						+'<td class="'+(case when days_until_expiration <= @critical_threshold_days then 'bg_red' else 'bg_orange' end)+'">'
									+convert(varchar,days_until_expiration)+'</td>'
						+'<td>'+isnull(convert(varchar,password_last_set_time,121),'')+'</td>'
						+'<td class="'+(case when is_expired = 1 then 'bg_red' else 'bg_none' end)+'">'
	...
	begin
		...
		begin try
			...
			;with t_login_info as (
				...
				inner join dbo.all_server_login_expiry_info_dashboard lei
					on lei.sql_instance = lex.sql_instance and lei.login_name = lex.login_name
				where lei.days_until_expiration <= @cto_threshold_days
				and left(lower(lex.login_name),6) <> 'bidba.'
			)
			,t_table_rows as (
				select	'<tr>'
